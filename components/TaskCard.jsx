...
const TaskCard = ({ task, onClick }) => (
  ...
  >
    <h3 className="font-bold text-lg text-white truncate">{task.part_name || 'Untitled'}</h3>
    <p className="text-sm text-gray-400 truncate">{task.company_name || 'Unknown Company'}</p>
    <div className="mt-4 flex items-center gap-2 text-xs text-gray-300">
      <MapPin size={14} />
      <span className="truncate">{task.location || 'Unknown Location'}</span>
    </div>
