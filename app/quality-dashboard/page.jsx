...
import { Input } from '@/components/ui'; // Import Input component
    ...
    const fetchTasks = async () => {
      try {
        ...
        if (response.success) {
          ...
          setFilteredTasks(response.tasks); // Initialize filteredTasks
        } else {
  ...
  const handleSearchChange = (e) => {
    ...
    if (term.trim() === '') {
      ...
      const filtered = tasks.filter(task => 
        task.partName?.toLowerCase().includes(term.toLowerCase())
      );
      setFilteredTasks(filtered);
    }
  };

  // Listen for new tasks from AddTaskPage (if redirected with state)
  useEffect(() => {
    // Check if there's a new task passed via router or some state mechanism
    // For simplicity, we assume the API call refreshes the task list
    // If you use Next.js router with state, you could handle it here
        ...
        <div className="flex-1 w-full mt-8">
          {(() => {
            ...
            if (filteredTasks.length === 0) {
              return <p className="text-center text-gray-400">No tasks found. Click the '+' button to add one.</p>;
            }
            return (
