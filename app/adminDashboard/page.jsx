...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async (role, setUsers) => {
      try {
        console.log(`Fetching ${role} users from ${process.env.NEXT_PUBLIC_API_URL}/api/users?role=${role}`);
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/users?role=${role}`, {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || `Failed to fetch ${role} users: ${response.statusText}`);
        }
  ...
  const handleSaveUser = async (userData) => {
    const token = localStorage.getItem('token');
    const url = modalMode === 'add' ? `${process.env.NEXT_PUBLIC_API_URL}/api/users/register` : `${process.env.NEXT_PUBLIC_API_URL}/api/users/${userData.id}`;
    ...
    try {
      console.log(`Saving user: ${method} ${url}`, userData);
      ...
      const targetStateSetter = userData.role === 'hod' ? setHodUsers : setEmployeeUsers;
